...
def capture_image(image_name):
    ...
    return f"data:AutonomousBefore/jpeg;base64,{encoded_image}"
    ...
    while True:
        ...
        if datos:
            ...
            if (front_left > 30 and front_right > 30) or (rear_left > 30 and rear_right > 30):
                ...
                for i in range(10):
                    delay1_3 = max(0.0005, min(0.001, 0.0005 - (i * 0.00009)))
                    delay2_4 = max(0.0005, min(0.001, 0.001 + (i * 0.00009)))
                ...
                for i in range(10):
                    delay1_3 = max(0.0005, min(0.001, 0.001 + (i * 0.00009)))
                    delay2_4 = max(0.0005, min(0.001, 0.0005 - (i * 0.00009)))
                    motor1.set_speed(delay1_3)
                    motor2.set_speed(delay2_4)
                    motor3.set_speed(delay1_3)
                    motor4.set_speed(delay2_4)
...
control_thread = threading.Thread(target=robot_controller)
...
try:
    ...
    emergency_stop()
    sensor_data = list(zip(*sensor_data))
    time_steps = range(len(sensor_data[0]))
