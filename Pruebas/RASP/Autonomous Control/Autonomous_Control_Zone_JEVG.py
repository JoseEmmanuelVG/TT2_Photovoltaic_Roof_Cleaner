...
class StepperMotor:
    shared_delay = 0.00025  # Variable compartida para el delay
...
def control_robot():
    error_count_left = 0
    error_count_right = 0
    last_error_left = False
    last_error_right = False
    ...
    while True:
        ...
        if datos:
            ...
            if (front_left > 30 and front_right > 30) or (rear_left > 30 and rear_right > 30) or (front_left > 30 and rear_left > 30) or (front_right > 30 and rear_right > 30):
                stop_all_motors()

            left_middle = sensor_distances[0]
            right_middle = sensor_distances[1]

            # Control PID para regresar a la línea
            if left_middle > 6.2 and rear_left > 27:
                correction = pid_left.compute(6.2, left_middle)
                ...
                error_count_left = 0
                last_error_left = False
            elif right_middle > 6.2 and rear_right > 27:
                correction = pid_right.compute(6.2, right_middle)
                motor1.shared_delay = max(0.0001, min(0.001, 0.0005 - correction))
                motor2.shared_delay = max(0.0001, min(0.001, 0.0005 + correction))
                motor3.shared_delay = max(0.0001, min(0.001, 0.0005 - correction))
                motor4.shared_delay = max(0.0001, min(0.001, 0.0005 + correction))
                ...
                motor1.shared_delay = 0.0005
                motor2.shared_delay = 0.0005
                motor3.shared_delay = 0.0005
                motor4.shared_delay = 0.0005

            # Verificar si debe moverse hacia adelante
            if left_middle < 6.2 and rear_left < 27 and right_middle < 6.2 and rear_right < 27:
                motor1.start_moving(True)
            ...
            if error_count_left >= 2:
                if front_left > 30:
                    print("El robot se movió hacia la derecha y dejó de seguir la línea.")
                else:
                    print("El robot se movió hacia la izquierda y dejó de seguir la línea.")
                stop_all_motors()
                error_count_left = 0

            if error_count_right >= 2:
                if front_right > 30:
                    print("El robot se movió hacia la izquierda y dejó de seguir la línea.")
                else:
                    print("El robot se movió hacia la derecha y dejó de seguir la línea.")
