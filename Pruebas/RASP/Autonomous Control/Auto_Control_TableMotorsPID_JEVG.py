...
def main():
    ...
    try:
        ...
        sensor_data_zipped = list(zip(*sensor_data))  # Transponer la lista para obtener los datos por sensor
        time_steps = range(len(sensor_data_zipped[0]))  # Crear una lista de pasos de tiempo

        plt.figure(figsize=(12, 8))
        plt.subplot(3, 2, 1)
        plt.plot(time_steps, sensor_data_zipped[0])
        plt.title('Delante izquierda')

        plt.subplot(3, 2, 2)
        plt.plot(time_steps, sensor_data_zipped[1])
        plt.title('Delante derecha')

        plt.subplot(3, 2, 3)
        plt.plot(time_steps, sensor_data_zipped[2])
        plt.title('Medio izquierda')

        plt.subplot(3, 2, 4)
        plt.plot(time_steps, sensor_data_zipped[3])
        plt.title('Medio derecha')

        plt.subplot(3, 2, 5)
        plt.plot(time_steps, sensor_data_zipped[4])
        plt.title('Atrás izquierda')

        plt.subplot(3, 2, 6)
        plt.plot(time_steps, sensor_data_zipped[5])
